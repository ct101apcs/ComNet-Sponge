...
TCPSender::TCPSender(const size_t capacity, const uint16_t retx_timeout, const std::optional<WrappingInt32> fixed_isn)
    ...
    , _elapsed_time(0)
    , _time_expired(0)
    , _retransmission_timeout(TCPConfig::TIMEOUT_DFLT)  
...
uint64_t TCPSender::bytes_in_flight() const {
    ...
    // uint64_t res = 0;
    // std::queue<std::pair<uint64_t, TCPSegment>> temp_queue = _outstanding_segments;
    // while (!temp_queue.empty()) {
    //     res += temp_queue.front().second.length_in_sequence_space();
    //     temp_queue.pop();
    // }
    // return res;
}
...
void TCPSender::fill_window() {
    while (true)
    {
        ...
        if (seg.length_in_sequence_space() > available_space)
            break;
        ...
        if (seg.length_in_sequence_space() > 0) {
            ...
            if (!_timer_running) {
                _time_expired = _elapsed_time + _retransmission_timeout;
                _timer_running = true;
            }
        }
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if (!_outstanding_segments.empty()) {
        auto seg = _outstanding_segments.front().second;
        seg.header().seqno = wrap(_next_seqno, _isn);
        _segments_out.push(seg);
        _timer_running = true;
        _time_expired = _elapsed_time + _retransmission_timeout;
...
void TCPSender::tick(const size_t ms_since_last_tick){
    ...
    if (_timer_running && _elapsed_time >= _time_expired) {
        // Timer expired, retransmit the oldest segment
        if (!_outstanding_segments.empty()) {
            auto seg = _outstanding_segments.front().second;
            seg.header().seqno = wrap(_next_seqno, _isn);
            ...
            // No segments to retransmit, stop the timer
            _timer_running = false;
            // _elapsed_time = 0;
        }
    }
